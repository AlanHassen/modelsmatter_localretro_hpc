...
from ssbenchmark.ssmodels.model_localretro import model_localretro

class LocalRetroSlurmLabeler:

    def __init__(self, training_templates_path, label_data_path, output_dir, split, retro = True, verbose = False, use_stereo = True, max_edit_n = 8, min_template_n = 1):
    ...
    def labeling_dataset(self, original_reaction_strings:list , processed_reactions:list, reaction_templates: list, training_template_dicts, training_template_infos):
        ...
        for i, (original_reaction_string, processed_reaction, reaction_template) in enumerate(zip(original_reaction_strings, processed_reactions, reaction_templates)):
            ...
            if edit_n <= self.max_edit_n:
                ...
                if i % 100 == 0:
                    print ('\r Processing %s data..., success %s data (%s/%s)' % (self.label_data_path, success, i, len(processed_reactions)), end='', flush=True)
            else:
        ...
        if self.split == 'train':
            print("Removing reactions with a frequency lower than %s" % self.min_template_n)
            
            # remove all rows with a frequency smaller than self.min_template_n
            df = df[df['Frequency'] >= self.min_template_n]
            # reset the index
            df = df.reset_index(drop=True)

        return df

    def load_reaction_strings_processed_reactions_reaction_templates(self):
    ...
    def run_labeling(self):
        ...
        reaction_strings, processed_reactions, reaction_templates = self.load_reaction_strings_processed_reactions_reaction_templates()
        ...
        print ('\nFinished labeling %s data!' % self.split)
...
if __name__ == '__main__':  
    ...
    parser.add_argument('-label', '--label_data_path', help='The saved labeled data')
    ...
    localRetroSlurmLabeler = LocalRetroSlurmLabeler(training_templates_path= args["training_template_path"], label_data_path = args["label_data_path"], output_dir= args["output_dir"], split = args["split"] , retro = args["retro"], verbose = args["verbose"], use_stereo = args["use_stereo"], max_edit_n = args["max_edit_n"], min_template_n = args["min_template_n"])
